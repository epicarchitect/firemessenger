...
import kolmachikhin.fire.messenger.core.viewmodel.auth.EmailAuthorizationState

@Composable
fun EmailAuthorization(
    state: EmailAuthorizationState,
    ...
    Box(
        ...
        when (state) {
            is EmailAuthorizationState.Input -> {
                val isEmailIncorrect = state is EmailAuthorizationState.Input.Incorrect && state.validatedEmail is Incorrect
                val isPasswordIncorrect = state is EmailAuthorizationState.Input.Incorrect && state.validatedPassword is Incorrect
                ...
                Column(
                    ...
                    AnimatedVisibility(visible = isEmailIncorrect) {
                        val incorrectEmail = (state as? EmailAuthorizationState.Input.Incorrect)?.validatedEmail as? Incorrect
                    ...
                    AnimatedVisibility(visible = isPasswordIncorrect) {
                        val incorrectPassword = (state as? EmailAuthorizationState.Input.Incorrect)?.validatedPassword as? Incorrect
                    ...
                    Button(
                        modifier = Modifier
                            ...
                            .padding(start = 54.dp, end = 54.dp, top = 16.dp),
                        onClick = {
                            (state as? EmailAuthorizationState.Input.Correct)?.startAuthorization?.invoke()
                        },
                        enabled = state is EmailAuthorizationState.Input.Correct
                    ) {
            ...
            is EmailAuthorizationState.Loading -> {
                Loading(stringResource(R.string.authorizing))
            }
            is EmailAuthorizationState.AuthorizationFailed -> {
                Column(
                    modifier = Modifier
            ...
            is EmailAuthorizationState.AuthorizationSuccess -> {
                /* no-op */
            }
        }
    }
